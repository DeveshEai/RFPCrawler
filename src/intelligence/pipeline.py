...
def is_crawl_running() -> bool:
    global _crawl_lock_active
    return _crawl_lock_active
...
class RFPIntelligencePipeline:
    ...
    async def run_pipeline(self, target_portal_id: str = None, auto_evaluate: bool = False) -> Dict[str, Any]:
        global _crawl_lock_active
        if _crawl_lock_active:
            system_logger.add_log("WARN", "⚠️ Crawl execution already active. Skipping duplicate concurrent trigger.")
            return {"scraped": 0, "stage1_passed": 0, "reverified": 0, "evaluated": 0, "pursued": 0, "emails_sent": 0, "status": "already_active"}

        _crawl_lock_active = True
        try:
            reset_crawl_cancel()
            self._ensure_portals_seeded()
            stats = {"scraped": 0, "stage1_passed": 0, "reverified": 0, "evaluated": 0, "pursued": 0, "emails_sent": 0}
            ...
            for adapter in active_adapters:
                ...
                for rfp_data in rfps:
                    ...
                    # Multi-field deduplication check (Pre-routing deduplication)
                    ...
                    if not is_clean_json:
                        ...
                        system_logger.add_log("INFO", f"[JSON-API] Clean pre-filtered IT notice bypassed Stage 1 keyword filter: '{rfp_data['title'][:40]}'")
